...
const fetchAPOD = async (
  ...
  try {
    ...
    const response = await fetch(
      `https://api.nasa.gov/planetary/apod?api_key=${import.meta.env.VITE_NASA_API_KEY}${dateParam}`
    );
    const data = await response.json();
    ...
    if (data.media_type === 'image') {
      ...
      // If today's APOD is not an image, try yesterday's
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);
      const yesterdayStr = yesterday.toISOString().split('T')[0];
      await fetchAPOD(setApodData, setLoading, yesterdayStr);
    }
  } catch (error) {
    console.error('Failed to fetch APOD:', error);
  } finally {
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center">
        <img
          ...
          onError={(e) => {
            console.error('Image failed to load:', apodData.url);
